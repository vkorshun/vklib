...
const
 { DefValues: array[0..13] of PChar = ( // unsafe type
  ...
  DefValues: array[0..13] of PChar = ( // unsafe type
    ...
    'Currency=гривна;гривни;гривень',
    'Cop=коп.;коп.;коп.',
    'Comma= ',
...
begin
  ...
  for i := 1 to Length(T) do begin // первый символ точно не пробел
    if (T[i] > #32) then begin
      ...
      if (not (CharInSet(T[i], PointSign)) and B) then Result := Result + #32;
      Result := Result + T[i];
    end;
    B := (T[i] <= #32);
  end;
...
begin
  ...
  if Length(Trim(GetValue('Cop', 1))) > 0  then begin
    ...
    S := Trim(S) + String(FComma) + ' ' + FormatFloat('00 ', L);
...
begin
  if FMale <> AMale then begin
    FMale := AMale;
    FInWordList.Values['CurrencyMale'] := String(CMale[FMale]);
    ResetText;
  end;
end;
...
begin
  AComma := ShortString(Trim(String(AComma)));
...
begin
  try // Нужно ли здесь делать Clear ???
    ...
    SetComma(ShortString(GetValue('Comma', 1)));
    ResetText;
  except
